...
// Offered in the category pulldown alongside whatever a company has already named, so filing a
// deliverable never starts with setting up a category. One of these becomes a real row the first
// time something is filed under it. They are suggestions, not a fixed list: categories are company
// data, a company can name them anything, and nothing in the code depends on these five existing.
export const SUGGESTED_CATEGORIES = ["BD & Sales", "Production", "Financial", "Operations & Systems", "Leadership"] as const;
